    ...
    <div class="services-breadcrumb">
      <div class="agile_inner_breadcrumb">
        <ul class="w3_short">
          ...
          <li>
            <router-link to="/personal">个人中心</router-link><span>|</span>
          </li>
          <li>文章创建</li>
        </ul>
      </div>
        ...
        <div class="row">
          <div class="col-md-12"
            ...
            <button type="button"
                    ...
                    @click="switchEditor"> 切换编辑模式 </button>
          </div>
        </div>
        <div class="row">
          <div class="col-md-3">
            <div class="article-base">
              ...
              <div class="form-group article-desc">
                ...
                <textarea class="form-control"
                          id="desc"
                          rows="4"
                          placeholder="请在此输入描述信息..."
                          v-model="articles.desc"></textarea>
              </div>
                ...
                <div class="row">
                  <div class="form-group"
                    ...
                    <label class="col-sm-4 control-label"
                           style="line-height: 34px;">分类</label>
                    <div class="col-sm-8">
                      <select class="form-control"
                              v-model="articles.category">
                        <option v-for="(cate, index) in category"
                                :value="cate.id"
                                :key="index">{{ cate.title }}</option>
                      </select>
                    </div>
                  </div>
                    ...
                    <div class="col-sm-8">
                      <select class="form-control"
                        ...
                        <option v-for="skill in skills"
                                :key="skill.sid"
                                :value="skill.sid">{{ skill.title }}</option>
                      </select>
                    </div>
                  </div><br />
                  ...
                  <div class="form-group">
                    <label class="col-sm-4 control-label"
                           style="line-height: 34px;">类别</label>
                    <div class="col-sm-8">
                      <select class="form-control"
                              v-model="articles.type">
                  ...
                  <div class="form-group">
                    <label class="col-sm-4 control-label"
                           style="line-height: 34px;">标签</label>
                    <div class="col-sm-8">
                    ...
                    <div class="col-sm-12">
                      <button class="btn btn-primary pull-right"
                              @click="addTag"><i class="fa fa-plus"></i> 添加类型</button>
                      <div style="clear: both;"></div>
                      <div class="tags"
                           id="tags">
                        <template v-for="(item, index) in tags">
                          <span class="ar-tag"
                                :key="item.id"><i class="fa fa-times-circle"
                               @click="tags.splice(index, 1)"></i> {{ item.title }} </span>
                        </template>
                      </div>
                    </div>
                  </div>
          ...
          <div class="col-md-9">
            <v-ckeditor ref="ck"
                        v-if="isCk"></v-ckeditor>
            <div class="mark-mavon"
                 v-if="!isCk">
              <mavon-editor :ishljs="false"
          ...
          <div class="col-md-12">
            <div class="article-button">
              <button type="button"
                      ...
                      @click="saveArticle"><i class="fa fa-save"></i> 保存 </button>
              <button type="button"
                      class="btn btn-primary"
                      @click="goBack"><i class="fa fa-mail-reply"></i> 返回 </button>
            </div>
          </div>
        </div>

        <div class="row">
          <v-toast v-show="showDialog"
...
//引入自定义组件
import upload_img from './components/shupload/upload.vue';
export default {
  data () {
    return {
      articles: {             //文章对象
  ...
  methods: {
    ...
    saveArticle: function () {
      ...
      this.articles.file = JSON.stringify(this.markFile);
      if (this.isCk) {
    ...
    $imgAdd (pos, $file) {                 // 绑定@imgAdd event
      ...
      this.$axios({
        url: '/api/index/blog/markdownUploadImage',
        method: 'post',
        data: formdata,
        headers: { 'Content-Type': 'multipart/form-data' },
      }).then((res) => {
         ...
         */
        this.markFile[res.data.value.file_name] = res.data.value.file_name;
        this.$refs.md.$img2Url(pos, res.data.value.view_path);
      })
    },
    $imgDel (pos) {
      //删除对应的图片
      var index = pos['0'].lastIndexOf('\/');
      let key = pos['0'].substring(index + 1, pos['0'].length);
      let filename = this.markFile[key];
      //console.log(filename);
      this.$axios({
        url: '/api/index/blog/markdownDelImage',
        method: 'post',
        ...
        delete this.markFile[key];
        //console.log(this.markFile);
      });
    }
  },
  mounted () {
